...
func (t *TimerDAO) UpdateTimer(ctx context.Context, timer *po.Timer, id uint) error {
	log.Infof("[UpdateTimer] timer:%v", utils.GetJsonStr(timer))
	return t.client.DB.WithContext(ctx).Where("id = ?", id).Updates(timer).Error
}
