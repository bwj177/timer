...
func (t *TimerApp) UpdateTimer(c *gin.Context) {
	var req vo.Timer
	if err := c.BindJSON(&req); err != nil {
		log.Errorf("[UpdateTimer] bind json err,err:%v", err)
		c.JSON(http.StatusBadRequest, vo.NewCodeMsg(-1, fmt.Sprintf("[create timer] bind req failed, err: %v", err)))
		return
	}

	log.Infof("[UpdateTimer] req:%v", utils.GetJsonStr(req))

	err := t.service.UpdateTimer(c.Request.Context(), &req, req.ID)
	if err != nil {
		c.JSON(http.StatusOK, vo.NewCodeMsg(-1, err.Error()))
		return
	}
	c.JSON(http.StatusOK, vo.NewUpdateTimerResp(vo.NewCodeMsgWithErr(nil)))
}
...
type timerService interface {
	...
	UpdateTimer(ctx context.Context, timer *vo.Timer, id uint) error
